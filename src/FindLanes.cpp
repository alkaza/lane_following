...
#else
		outImg = cv::Mat::zeros(frameImg.size(), frameImg.type());
#endif
	}
}
...
void FindLanes::CalcPoly(LANE_MODE mode) {
	std::vector<cv::Point> &lane_pts =
...
void FindLanes::Steering() {
	...
	if (bVerbose) {
...
#if 0
		warpPerspective(outImg, outImg, Minv, outImg.size());
		addWeighted(outImg, 0.4, frameImg, 0.6, 0, outImg);
#endif

		// Frame duration indicator
		float pix_p_frame = speed_pix_p_us * frameDuration;
		float pix_p_frame_y1 = img.rows - pix_p_frame;
		float pix_p_frame_y2 = img.rows - pix_p_frame * 2;
		float pix_p_frame_y3 = img.rows - pix_p_frame * 3;

		cv::line(outImg, cv::Point(5, img.rows - 1),
				cv::Point(5, pix_p_frame_y1), cv::Scalar(0, 255, 0), 10);
		cv::line(outImg, cv::Point(5, pix_p_frame_y1),
				cv::Point(5, pix_p_frame_y2), cv::Scalar(0, 255, 255), 10);
		cv::line(outImg, cv::Point(5, pix_p_frame_y2),
				cv::Point(5, pix_p_frame_y3), cv::Scalar(0, 0, 255), 10);
		...
		std::stringstream ss;
		ss.precision(3);
		ss << std::left << "FrameIndex[" << frameIndex << "], Speed="
				<< std::setw(4) << speed_m_p_s << "m/s, Angle =" << std::setw(6)
				<< angle_deg << "deg, Offset=" << std::setw(6)
				<< offset / img.cols / 1.5 << "m, LaneWidth=" << std::setw(4)
				<< laneHistory.laneWidth / img.cols / 1.5 << "m, FrameDuration="
				<< std::setw(6) << (double) frameDuration / 1000000
				<< "s, LineFound:" << std::setw(1)
				<< (leftLine.found && rightLine.found ? "2" :
					leftLine.found ? "L" : rightLine.found ? "R" : "0");
		std::string text = ss.str();
		cv::Size textSize = cv::getTextSize(text, CV_FONT_HERSHEY_SIMPLEX, 0.6,
				2, 0);
		cv::Point textOrg(outImg.cols - textSize.width, textSize.height);
		putText(outImg, text, textOrg, CV_FONT_HERSHEY_SIMPLEX, 0.6,
				cv::Scalar::all(255), 2);
	}
}
